...
from afsfc.algorithms import clustering_algorithms
from afsfc.utils.string_utils import encode_parameter, decode_parameter
...
class Mapper(object):
    d = {
        class_name: class_obj for class_name, class_obj in clustering_algorithms.__dict__.items()
    ...
    for idx, string in enumerate(itertools.chain(clustering_ls, dim_reduction_ls)):
        ...
        for param in algorithm.params:
            cs.add_condition(InCondition(child=param,
                                         parent=clustering_choice if idx < len(clustering_ls) else dim_reduction_choice,
                                         values=[string]))
